...
//       将属性设置给窗体
    ...
    public void onClick(View view) {
        int id = view.getId();
        if (id == R.id.tv_saoma) {
            Intent intent = new Intent(ScanActivity.this, QRScanActivity.class);
            startActivityForResult(intent, REQUEST_CODE_SCAN1);
        } else if (id == R.id.tv_shoudong) {//                Toast.makeText(this,"点击了从相册选择",Toast.LENGTH_SHORT).show();
            ...
            new AlertDialog.Builder(this).setTitle("请输入设备号").setIcon(
                ...
                public void onClick(DialogInterface dialog, int which) {
                    ...
                    SipURL sipURL = new SipURL(devid, SipInfo.serverIp, SipInfo.SERVER_PORT_USER);
                    SipInfo.toDev = new NameAddress(devName, sipURL);
                    org.zoolu.sip.message.Message query = SipMessageFactory.createNotifyRequest(sipUser, SipInfo.toDev,
                            SipInfo.user_from, BodyFactory.createAdddevNotify(devid, SipInfo.userId));
                    sipUser.sendMessage(query);
