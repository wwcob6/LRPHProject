...
public class PhoneCall extends BaseActivity implements SipService.AudioCallListener {
    @BindView(R2.id.phoneNum)
    TextView phoneNum;
    @BindView(R2.id.status)
    TextView status;
    @BindView(R2.id.time)
    Chronometer time;
    @BindView(R2.id.speakerMode)
    Button speakerMode;
    @BindView(R2.id.layout1)
    LinearLayout layout1;
    @BindView(R2.id.accept)
    ImageButton accept;
    @BindView(R2.id.decline)
    ImageButton decline;
    @BindView(R2.id.layout2)
    RelativeLayout layout2;
    @BindView(R2.id.endCall)
    ImageButton endCall;
    ...
    private void init() {
        ...
        if (type == 1) {
            ...
            mWakelock = powerManager.newWakeLock(PowerManager.ACQUIRE_CAUSES_WAKEUP |
                    WindowManager.LayoutParams.FLAG_KEEP_SCREEN_ON, "app:tag");
            mWakelock.acquire();
            status.setText("来电");
    ...
    @OnClick({R2.id.speakerMode, R2.id.accept, R2.id.decline, R2.id.endCall})
    public void onClick(View view) {
        int id = view.getId();
        if (id == R.id.speakerMode) {
            if (isSpeakerMode) {
